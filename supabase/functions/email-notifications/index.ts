...
// CANONICAL_DOMAIN must be set as a Supabase secret with the SAME value as
// the frontend's VITE_CANONICAL_DOMAIN (see .env.example) — one domain,
// configured once, referenced everywhere. No more per-file hardcoded URLs.
const CANONICAL_DOMAIN = Deno.env.get('CANONICAL_DOMAIN') ?? 'https://bird-validation-survey.bolt.host';
const SITE_URL       = Deno.env.get('SITE_URL')       ?? CANONICAL_DOMAIN;
const USER_MANUAL    = Deno.env.get('USER_MANUAL_URL') ?? `${CANONICAL_DOMAIN}/user-manual.html`;
