...
// Updated: Strict alignment with Section 9 reference architecture
...
export const Section11_Metrics: React.FC<Section11Props> = ({ data, onChange }) => {
  ...
  const archetypes = ARCHETYPES_BY_SECTION[11] ?? [];
  const driftingGoalsAgree =
    data.q_s11_drifting_goals === "Accurate" ||
    data.q_s11_drifting_goals === "Somewhat Accurate";
  ...
  return (
    ...
    >
      ...
      <SectionProgress 
        currentSection={11} 
        totalSections={16} 
        sectionLabel="Metrics Architecture" 
      />
      ...
      <motion.div variants={cardVariants}>
        <Card className="border-[#C9A84C]/20 bg-white/95 dark:bg-[#022c22]/80 backdrop-blur-sm shadow-sm">
          ...
          <CardContent className="space-y-6">
            ...
            <p className="text-sm text-[#022c22] dark:text-[#ecfdf5]/90 leading-relaxed pt-2">
            ...
            <div className="pt-4 border-t border-[#C9A84C]/10 space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#ecfdf5] mb-2">
                  How accurately does the &quot;Drifting Goals&quot; archetype illustrate BARMM&apos;s development ambitions weakening over time?
                </label>
              ...
              {driftingGoalsAgree && (
                <div className="space-y-3 pt-4 border-t border-[#C9A84C]/20 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    {[
                      ...
                      <button
                        ...
                        className={cn(
                          "p-3 rounded-lg border text-xs text-left transition-all flex items-center gap-2",
                          data.q_s11_drifting_goals_followup === opt
                            ? "bg-[#C9A84C]/15 border-[#C9A84C] text-[#E5C560]"
                            : "bg-[#022c22]/40 border-white/10 text-[#ecfdf5]/70 hover:border-[#C9A84C]/30"
                        )}
                      >
                  ...
                  <input
                    ...
                    value={
                      data.q_s11_drifting_goals_followup &&
                      ![
                        "B1: Persistent performance gaps create pressure to reduce targets, temporarily easing institutional stress",
                        "B2: Agencies respond with short-term fixes — training programs, subsidies, or ad-hoc reforms — but these rarely close the structural gaps",
                        "Both loops are equally observable",
                      ].includes(data.q_s11_drifting_goals_followup)
                        ? data.q_s11_drifting_goals_followup
                        : ""
                    }
        ...
        <div className="relative rounded-2xl overflow-hidden border border-[#C9A84C]/20 shadow-xl">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <p className="text-xs text-[#ecfdf5]/70 italic">
              {BIRD_IMAGES.metricsArchitecture?.description || "The comprehensive 5-tier calibration architecture and cross-cluster synchronization framework for BIRD 2026-2035."}
            </p>
          </div>
        </div>
      </motion.div>
